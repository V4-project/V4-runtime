                                    ...
                                    rmt_encoder_handle_t *ret_encoder)
{
  ...
  // different led strip might have its own timing requirements, following parameter is
  // for WS2812
  ...
  ESP_GOTO_ON_ERROR(
      rmt_new_bytes_encoder(&bytes_encoder_config, &led_encoder->bytes_encoder), err, TAG,
      "create bytes encoder failed");
  rmt_copy_encoder_config_t copy_encoder_config = {};
  ESP_GOTO_ON_ERROR(
      rmt_new_copy_encoder(&copy_encoder_config, &led_encoder->copy_encoder), err, TAG,
